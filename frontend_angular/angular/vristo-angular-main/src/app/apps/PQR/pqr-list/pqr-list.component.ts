...
export class PqrListComponent implements OnInit {
    ...
    rows = [
        {
            ...
            description: 'Caroline Jensen solicita actualizar su dirección a 529 Scholes Street, Temperanceville, código postal 5235.',
            status: 'Pending',
            date: '2024-02-18',
            type: 'Petition',
            priority: 'Low',
        },
        {
            ...
            description: 'Celeste Grant reporta que su servicio está inactivo desde el 19/11/2023. Solicita reactivación inmediata.',
            status: 'In Progress',
            date: '2024-02-17',
            type: 'Complaint',
            priority: 'High',
    ...
    verInformacion(value: any) {
        this.selectedPqr = value;
        this.isViewMode = true;
        this.isEditMode = false;
    }

    editarInformacion(value: any) {
        this.selectedPqr = value;
        this.isEditMode = true;
        this.isViewMode = false;
    }

    eliminarInformacion(value: any) {
        // Implementar lógica para eliminar
        console.log('Eliminar información:', value);
        // Ejemplo: this.rows = this.rows.filter(row => row.id !== value.id);
    }

    closeModal() {
        this.isViewMode = false;
        this.isEditMode = false;
        this.selectedPqr = null;
    }

    savePqr(updatedPqr: any) {
        // Actualizar el PQR en la lista
        const index = this.rows.findIndex((row) => row.id === this.selectedPqr.id);
        if (index !== -1) {
            this.rows[index] = { ...this.rows[index], ...updatedPqr };
        }
        this.closeModal();
    }
}
